...
;(function () {
	...
	co.star = function (a, b, c, s) {
		const T = !isNaN(s) ? 'marquee' : 'shape', o = co.constrain(a, b, s), scale = co.scale(a, b, s)
		const rad = Math.sqrt(Math.pow(a.X - o.X, 2) + Math.pow(a.Y - o.Y, 2)) / 2, rad2 = rad / vars['slope_' + T],
			sides = vars['sides_' + T]
	...
	co.gradient = function (a, b, c, r, v, s) {
		let g
		if (!s) { // ROTATE
			const W = Math.sqrt(Math.pow(b.X - a.X, 2)),
				W2 = W / 2,
				rad = gui_gradient.rotate,
				mX = Math.min(b.X, a.X),
				mY = Math.min(b.Y, a.Y)
			const s1 = Math.abs((Math.sin(rad) * W2) + W2),
				s2 = Math.abs((Math.cos(rad) * W2) - W2)
			const x1 = Math.abs((Math.sin(rad + Math.PI) * W2) + W2),
