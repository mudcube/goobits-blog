...
function findEXIFinJPEG(oFile) {
	...
	if (oFile.charCodeAt(0) != 0xFF || oFile.charCodeAt(1) != 0xD8) {
	...
	while (iOffset < iLength) {
		if (oFile.charCodeAt(iOffset) != 0xFF) {
			if (bDebug) console.log("Not a valid marker at offset " + iOffset + ", found: " + oFile.charCodeAt(iOffset));
			return false; // not a valid marker, something is wrong
		}

		var iMarker = oFile.charCodeAt(iOffset + 1);
...
function readTags(oFile, iTIFFStart, iDirStart, oStrings, bBigEnd) {
	...
	for (var i = 0; i < iEntries; i++) {
		...
		if (!strTag && bDebug) console.log("Unknown tag: " + getShortAt(oFile, iEntryOffset, bBigEnd));
...
function readTagValue(oFile, iEntryOffset, iTIFFStart, iDirStart, bBigEnd) {
	...
	case 7:
		...
		if (iNumValues == 1) {
			return oFile.charCodeAt(iEntryOffset + 8, bBigEnd);
		} else {
			var iValOffset = iNumValues > 4 ? iValueOffset : (iEntryOffset + 8);
			var aVals = [];
			for (var n = 0; n < iNumValues; n++) {
				aVals[n] = oFile.charCodeAt(iValOffset + n);
			}
			return aVals;
		}
		break;
...
function readEXIFData(oFile, iStart, iLength) {
	if (getStringAt(oFile, iStart, 4) != "Exif") {
		if (bDebug) console.log("Not valid EXIF data! " + getStringAt(oFile, iStart, 4));
		return false;
	}
	...
	if (getLongAt(oFile, iTIFFOffset + 4, bBigEnd) != 0x00000008) {
		if (bDebug) console.log("Not valid TIFF data! (First offset not 8)", getShortAt(oFile, iTIFFOffset + 4, bBigEnd));
		return false;
	}
...
function getStringAt(file, iOffset, iLength) {
	var aStr = "";
	for (var j = 0; j < iLength; j++) {
		var bytes = file.charCodeAt(iOffset + j);
		if (bytes == 0) return str;
		aStr += String.fromCharCode(bytes);
	}
	return aStr;
}

function getShortAt(file, iOffset, bBigEndian) {
	var iShort = bBigEndian ? 
		(file.charCodeAt(iOffset) << 8) + file.charCodeAt(iOffset + 1)
		: (file.charCodeAt(iOffset + 1) << 8) + file.charCodeAt(iOffset)
	if (iShort < 0) iShort += 65536;
	return iShort;
}

function getSLongAt(file, iOffset, bBigEndian) {
	var iULong = getLongAt(file, iOffset, bBigEndian);
...
function getLongAt(file, iOffset, bBigEndian) {
	var iByte1 = file.charCodeAt(iOffset),
		iByte2 = file.charCodeAt(iOffset + 1),
		iByte3 = file.charCodeAt(iOffset + 2),
		iByte4 = file.charCodeAt(iOffset + 3);
