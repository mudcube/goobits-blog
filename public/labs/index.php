...
$hide = array(
			'dlf',
			'Thumbs',
			'Unicode-Profiling-Project',
			'Font-Profiling-Project',
			'Color-Piano-Theory',
			'Icon',
			'.DS_Store',
			'.htaccess',
...
if ($handle = opendir($opendir)) {
	while (false !== ($file = readdir($handle))) { 
		...
		if(!$leadon && $file == "index.php") continue;
		if($discard) continue;
		$type = @filetype($leadon.$file);
...
<head>
 <title>mudcu.be<?php echo str_replace("/"," > ", dirname($_SERVER['PHP_SELF']).'/'.$leadon);?></title>
 <link rel="icon" href="../media/favicon.png" type="image/x-icon" />
 <link href="../media/main.css" rel="stylesheet" type="text/css" />
 <link rel="stylesheet" type="text/css" href="../media/dir-listing/styles.css" />
<style type="text/css">
body { 
	background: #000518;
...
<div id="container" style="padding-top: 80px">
  <div id="breadcrumbs"> <a href="<?php echo $_SERVER['PHP_SELF'];?>">Laboratory</a> 
  <?php
  ...
  <div id="listingcontainer">
	...
	<div id="headermodified"><a href="<?php echo $baseurl . $dateurl;?>">Date Modified</a></div>
	</div>
    <div id="listing">
	<?php
	$class = 'b';
	if($dirok) {
	?>
	<div><a href="<?php echo $_SERVER['PHP_SELF'].'?dir='.urlencode($dotdotdir);?>" class="<?php echo $class;?>"><img src="../media/dir-listing/dirup.png" alt="Folder" /><strong>..</strong> <em>-</em> <?php echo date ("M d Y h:i:s A", filemtime($dotdotdir));?></a></div>
	<?php
		if($class=='b') $class='w';
		else $class = 'b';
	}
	...
	for($i=0;$i<$arsize;$i++) {
	?>
	<div><a href="<?php echo (file_exists($leadon.$dirs[$i] . "index.html") || file_exists($leadon.$dirs[$i] . "index.php")) ? $leadon.$dirs[$i] : $_SERVER['PHP_SELF'].'?dir='.urlencode($leadon.$dirs[$i]);?>" class="<?php echo $class;?>"><img src="../media/dir-listing/folder.png" alt="<?php echo $dirs[$i];?>" /><strong><?php echo $dirs[$i];?></strong> <em>-</em> <?php echo date ("M d Y h:i:s A", filemtime($leadon.$dirs[$i]));?></a></div>
	<?php
		if($class=='b') $class='w';
		else $class = 'b';	
	}
	...
	for($i=0;$i<$arsize;$i++) {
		...
		if (filetype($files[$i]) == "link") $ext = "link";
		$supportedimages = array('gif', 'png', 'jpeg', 'jpg');
		$thumb = '';
		
		if($showthumbnails && in_array($ext, $supportedimages)) {
			$thumb = '<span><img src="../media/dir-listing/trans.gif" alt="'.$files[$i].'" name="thumb'.$i.'" /></span>';
	...
	?>
	<div><a href="<?php echo $fileurl;?>" class="<?php echo $class;?>"<?php echo $thumb2;?>><img src="../media/dir-listing/<?php echo $icon;?>" alt="<?php echo $files[$i];?>" /><strong><?php echo $filename;?></strong> <em><?php echo round(filesize($leadon.$files[$i])/1024);?>KB</em> <?php echo date ("M d Y h:i:s A", filemtime($leadon.$files[$i]));?><?php echo $thumb;?></a></div>
	<?php
		if($class=='b') $class='w';
		else $class = 'b';	
	}	
