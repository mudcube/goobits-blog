...
*/ if (typeof Canvas2D == "undefined") var Canvas2D = {};
(function() {
    var surface = {}; // canvas array
    Canvas2D = mixin(Canvas2D, {
        add: function(attr, height, id, style) {
            if (typeof attr == "number") attr = {
                width: attr,
                height: height,
                id: id,
                style: style
            ...
            if (attr) {
                ...
                if (attr && attr.id && surface[attr.id]) return surface[attr.id];
            }
            var d = document.createElement("canvas"), ctx = d.getContext("2d");
            if (typeof attr == "object") for(var key in attr)switch(key){
                case "style":
                    if (typeof attr[key] == "object") {
                        for(var type in attr[key])d.style[type] = attr[key][type];
                        break;
                    } else {
            ...
            if (attr.id) surface[attr.id] = ctx;
            return ctx;
        },
        remove: function(id) {
...
//# sourceMappingURL=index.869e5152.js.map
