...
style = function(c, style, type, a, b) {
    switch(vars[type]){
        case "solid":
            ...
            break;
        case "gradient":
            var r = vars[type + "GD"], gradient = c.createLinearGradient(a.X, a.Y, b.X, b.Y);
            for(var key in r)gradient.addColorStop(r[key][0], "rgba(" + r[key][1].join(",") + ")");
            c[style] = gradient;
            break;
        case "pattern":
            if (!vars[type + "PT~"]) gui_pattern.cache(type);
...
(function() {
    // helper functions
    function createFlow(b1, type, callback) {
        var oX = b1.X - cXY.X, oY = b1.Y - cXY.Y, flow = Math.max(1, 100 - vars["flow_" + type]);
        function run(a, b, n) {
            var i = n / flow;
            if (n > 0) for(; i > 0; i--)callback(a * i, b * i);
            else for(; i < 0; i++)callback(a * i, b * i);
        }
        if (Math.abs(oX) > Math.abs(oY)) run(flow, flow * (oY / oX), oX);
        else run(flow * (oX / oY), flow, oY);
    }
    function mask_active(m) {
        var c = $2D("ctx_temp"), type = vars.type == "shape" ? vars.shape : vars.type;
        c.globalCompositeOperation = canvas.mode;
        if (vars["movement_" + type] == "anchored") c.clearRect(0, 0, canvas.W, canvas.H);
    ...
    draw = {
        shape: function(a, b, m) {
            var c = mask_active(m);
            if (vars["movement_" + vars.shape] == "freedraw") oXY = b;
        ...
        pencil: function(a, b, m) {
            ...
            if (m == "down") z(0, 0);
            else if (m == "move") createFlow(b, "pencil", z);
            if (m == "up") mask_up(c);
        },
        brush: function(a, b, m) {
            ...
            if (m == "down") z(0, 0);
            else if (m == "move") createFlow(b, "brush", z);
            if (m == "up") mask_up(c);
        },
        ...
        preview: function(i, m) {
            if ($("stamp" + i) && stamp.src[i].src) {
                ...
                if (i == stamp.fileNumber) {
                    ...
                    c.globalCompositeOperation = "source-over";
                    if (!m || m == "up") co.glyph(stamp.uri("live"));
                }
            }
        },
        reset: function() {
            var o = gui.Y, r = o.r.stamp;
            if (!stamp.fileNumber) stamp.fileNumber = 1;
            var a = parseInt(stamp.fileNumber), b = stamp.r[vars.stamp], i = r.n() - r.display, n = a / b <= 0.5 ? -2 : 2;
            o.prev.stamp = null;
    ...
    (function() {
        ...
        draw.spirograph = function(a, b, m) {
            ...
            if (m == "move") o = b;
                ...
                o = b;
                var R = vars.inner_radius_spirograph, r = vars.outer_radius_spirograph, d = vars.diameter_spirograph, speed = vars.speed_spirograph, resolution = vars.resolution_spirograph, stroke = 0.5, type = vars.type_spirograph, theta = 0, strokeStyle = vars["stroke" + gui_swatch.L2S[vars["stroke"]]];
                switch(vars["stroke"]){
                    case "pattern":
                        c.strokeStyle = vars["strokePT~"];
                        break;
                    case "gradient":
                        var Additive = function(A, B) {
                            var C = A + B;
                            return C > 0xFF ? 0xFF : C;
                        }, gradient = vars["strokeGD"], prev, cur, spectrum = [];
...
//# sourceMappingURL=index.edd184ef.js.map
