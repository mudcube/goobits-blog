/*
	----------------------------------------------------
	Uploader.js : 0.4 : 2012/06/15 : http://mudcu.be
	----------------------------------------------------
	...
	new widgets.Uploader({ 
		...
		fakeInput: image,
		formats: "ggr,png",
		onChange: function(files, self) {

		}
	});
	// Example of upload from external source.
	new RCTID.uploader.upload({
...
widgets.Uploader = function(conf) {
	...
	this.maxFileSize = conf.maxFileSize || 200000000; // Default 200MB.
	...
	this.dropArea = conf.dropArea || document.body; // Element to initialize for dropping.
	...
	var createDropArea = function() {
		...
		dropArea.appendChild(document.createTextNode(that.dropAreaMessage));
		...
		var getNextFile = function() {
			...
			if (++ idx > that.maxFiles || !file) { // When the queue is complete.
				if (that.maxFiles === 1) {
					...
					if (!that.files[key]) return that.onError("UPLOAD_ERR_FORMAT", that);
					return that.onChange(tmp, that);
				} else {
					for (var key in that.files);
					if (!that.files[key]) return;
					return that.onChange(that.files, that);
				}
			}
			...
			if (!that.formats[ext]) return getNextFile(); 

			// Check whether file is empty.
			if ((file.fileSize || file.size) === 0) return getNextFile();	
...
//				data.append(file.name + (file.size || ""), content);
		...
		xhttp.onerror = function (event) {
			if (that.onError) that.onError("UPLOAD_ERR_XHTTP", self);
		};
...
//				input.name = file.name + (file.size || "");
				input.name = file.name;
				form.appendChild(input);
			} else { // uploading a file.
	...
	///
	if (this.dropArea) createDropArea();
	if (this.fakeInput || this.fileInput) createFileInput();
	///
	return this;	
};