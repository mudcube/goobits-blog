/* GUI */ gui = {
    ...
    "options": function() {
        ...
        for(var i1 in r1)cF["movement_" + r1[i1]] = {
            type: "radio",
            val: [
                "movement_" + r1[i1],
                "anchored",
        ...
        var j = 0, r1 = [];
        for(var i1 in stamp.r)r1[j++] = i1;
        cF.stamp = {
            type: "menu",
            val: r1
        };
        if (!gui.menu.cur) {
            if (!vars.crop) vars.crop = "Original (" + fu(canvas.W) + "x" + fu(canvas.H) + ")";
            gui.menu.cur = {};
            gui.menu.key = {};
            ...
            for(var i1 in gui.menu.key)gui.menu.cur[i1] = String(gui.menu.key[i1]);
    ...
    "radio": {
        ...
        "click": function(o, v) {
            if (!$C("cur", o.parentNode)[0]) return;
            ...
            if (v == "marquee") {
                ...
                if (i == "lasso" || cur == "lasso") marquee.reset("", 1);
    ...
    "menu": {
        "build": function(c, r) {
            ...
            if (typeof r == "object" && !r.length) for(var i in r)length++;
        ...
        "fu": {
            ...
            "PT*": function(c, o1) {
                gui.menu.fu.z(c, o1);
                vars.PT = Q.PT[o1.innerHTML];
                var i = vars.id + "PT", n = vars.PT.length - (gui_swatch.n[i] = Math.min(gui_swatch.n[i], vars.PT.length));
        ...
        "klean": function(v, o) {
            gui.menu.cur[v] = o;
            vars[v] = o;
            var a = o, b = a.substr(0, a.indexOf("(") != -1 ? a.indexOf("(") - 1 : a.length);
            gui.menu.key[v] = b;
        },
        // Data
        "cellHeight": 17,
    ...
    "Y": {
        ...
        "wheel": function(event) {
            ...
            if (event.wheelDelta) {
                ...
                if (window.opera) v = -v;
            // OPERA + IE
            } else if (event.detail) var v = -event.detail / 3;
            var n = Math.max(2.5, r.n() / (r.col * r.row) / r.row * 4), v = v <= 0 ? Math.floor(v + 0.2) : Math.ceil(v + 0.5); //- JUNK
            o.cord(v * n);
            eval(o.fu[o.id] + "()");
        },
        "cord": function(v) {
        ...
        "sw": function(s) {
            ...
            if (r2.active($(gui_swatch.id), 7, c[gui_swatch.id], c[gui_swatch.id] + 27, {
                "each": fu,
                "vars": {
                ...
                if (gui_swatch.id == "PT") {
                    for(var i2 = c[gui_swatch.id]; i2 <= c[gui_swatch.id] + 27; i2++)if (i2 <= vars[gui_swatch.id].length && vars.PT[i2 - 1]) vars.PT[i2 - 1].onload = function() {
        ...
        "stamp": function(s) {
            ...
            if ($("brush_author")) {
                ...
                if (!stampSet) $("brush_author").innerHTML = "";
                else $("brush_author").innerHTML = '<i style="-moz-user-select: none; -khtml-user-select: none; user-select: none; ">by:&nbsp; <a href="' + stampSet.url + '" target="_blank">' + stampSet.name + '</a></i><div style="background: #555; height: 1px; margin: 6px 0 2px; "></div>';
            }
            if (r3.active($("stamp"), 4, c[r3.id], c[r3.id] + 11, {
                "change": function() {
                    stamp.src = [];
                },
                "each": fu,
            ...
            }, s)) for(var i3 in stamp.src)stamp.src[i3].onload = function() {
                stamp.preview(this.id);
            };
        },
        "hi": function(s) {
            var r4 = gui.Y, c = r4.cur;
            r4.id = "hi";
            function fu(i) {
                var r = canvas.history_r;
                i--;
            ...
            r4.active($C("z", "history")[0], 1, c[r4.id], c[r4.id] + 7, {
                "each": fu
            }, s);
        }
    },
    ...
    "X": {
        ...
        "xRun": function(o, e) {
            core.fu(o + "Cur", e, {
                ...
                var i = cF[o];
                if (i.fu) i.fu(o, i.val, b.X, m);
                else gui.X.html(o, i.val, b.X / 110, m);
                gui.X.left(o, b.X);
        ...
        "xxSwitch": function(o, e, v) {
            ...
            var a1 = zero($S(o + "_maxCur").left), b1 = zero($S(o + "_minCur").left);
            if (Math.abs(n - a1) < Math.abs(n - b1) || Math.abs(n - a1) == Math.abs(n - b1) && n <= a1) z("_max", "_min");
            else z("_min", "_max");
        },
        "range": function(v, i, n) {
            var a = i == "_min" ? n : parseInt($S(v + "_minCur").left), z = i == "_max" ? n : parseInt($S(v + "_maxCur").left);
            $S(v + "cCur").left = z + 7 + "px";
            $S(v + "cCur").width = Math.max(0, a - z - 7) + "px";
        },
        "xxRun": function(a, b2, m, i4) {
            ...
            if (i4 == "_max" && zero($S(v + "_minCur").left) < b2.X + 1) z("_min", Math.min(115, b2.X + 1));
            else if (i4 == "_min" && zero($S(v + "_maxCur").left) > b2.X - 1) z("_max", Math.max(0, b2.X - 1));
            z(i4, b2.X);
        },
        "xxID": 0
    }
...
win = {
    ...
    "close": function(v, s) {
        ...
        if (s) setTimeout("var v=$('i" + v + "'); v.className=(v.className=='cur')?'':'cur'; v.opened=false;", 190);
    },
    "tab": function(o, v) {
        var d = $(o.id.substr(1));
        if (o.opened == false) {
    ...
    "cp": function(o, r) {
        ...
        for(var i in win.r)z += i + "," + win.r[i] + ":";
        if (o) cookieStab("windows", z);
        else return z;
    },
    // Visualizer
    "apply": function(o, r) {
    ...
    "go": function(o, r, n) {
        ...
        o.interval = window.setInterval(function() {
            var done = true, A = [], Z = [];
            for(i in r)A[i] = Math.round((r[i] - o[i]) * n);
            for(i in A){
                Z[i] = o[i] + A[i];
                if (A[i] == 0 || A[i] > 0 && Z[i] > r[i] || A[i] < 0 && Z[i] < r[i]) Z[i] = r[i];
                else done = false;
            }
            win.apply(o, Z);
            if (done) {
...
win_size = {
    ...
    "core": function(e) {
        function fu(o, m, a) {
            var Y = Math.max(100, a.Y - r.Y + r.H), X = Math.max(212, a.X - r.X + r.W);
            var o = {
                W: X - 32,
                H: Y - 40
            };
...
//# sourceMappingURL=index.56652fcf.js.map
