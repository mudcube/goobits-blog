...
gui_tools = {
    imagePos: {},
    imageMap: function() {
        var elements = $T("div", $C("tools", "tools")[0]), icons = [
            "Brush",
            "Calligraphy",
            "Crop",
            "Eraser",
        ...
        for(var n in icons)gui_tools.imagePos[icons[n]] = n * 26 - 5;
        for(var image in elements){
            ...
            if (typeof image == "object") {
                ...
                if (o[0]) {
                    ...
                    var u = o.split("_")[0];
                    if (u == "Marquee" || u == "Shape") o = u + "_" + vars[u.toLowerCase()];
                    else if (u == "Brush") o = ucword(vars["draw"]);
    ...
    imageSize: function(v, i, end) {
        ...
        if (i == end) {
            ...
            if (o[1]) o[0].style.display = i == 0 ? "none" : "block";
        }
    },
    imageCurrent: function(v1) {
        function getRoot(v) {
            return (v.indexOf("_") != -1 ? v.substr(0, v.indexOf("_")) : v).toLowerCase();
        }
        var prev = gui_tools.prev, p = getRoot(prev ? prev : vars.type), c = getRoot(v1);
        vars.type = c;
        vars.typeImg = v1;
        if (prev != v1) {
            function zoom(o, n) {
            ...
            if (c == "marquee" || c == "crop") marquee.reset();
            if (p == "crop") crop.reset();
            if (prev) {
                var timer = 0, img1 = new Image();
                img1.src = "media/gui/tools/" + prev + ".png";
                for(var i = 25; i >= 0; i--)zoom(prev, 0);
            }
            var timer = 0, img2 = new Image();
            img2.src = "media/gui/tools/" + v1 + "_2.png";
            for(var i = 0; i <= 25; i++)zoom(v1, 25);
            gui_tools.prev = v1;
...
gui_options = {
    ...
    forge: function(v) {
        if (!gui_options.modules || v == "shape") gui_options.modulesObject();
        var r = gui_options.modules[v], z = "";
        for(var i in r.vars){
            if (!vars[i]) var b = r.vars[i];
            if (i == "movement_shape") i = "movement_" + vars.shape;
            else if (i == "stroke_shape") i = "stroke_" + vars.shape;
            if (!vars[i]) vars[i] = b;
        }
        if (v == "shape" || v == "marquee") {
            var d = vars[v];
            gui_options.resize(r.size[d] ? r.size[d] : r.size[""]);
        } else gui_options.resize(r.size);
        for(var i in r.build){
            i = r.build[i];
            var o = i.indexOf("_") != -1 ? i.substr(0, i.indexOf("_")) : i, title = o.replace("_", " ").replace("line", "").replace("rand", "diameter &middot; min, max").replace("font", "").replace("marq", "").replace("shape", "").toLowerCase(), fu = i == "br" ? "<br>" : gui[cF[i].type].build(title, i, cF[i].val);
            if (v == "shape") {
                if (d != "ellipses" && d != "polygon" || d == "ellipses" && i != "corner" && i != "sides_shape" && i != "slope_shape" || d == "polygon" && i != "slope_shape") z += (i == "stroke_ellipses" ? "<br>" : "") + fu;
            } else if (v == "marquee") {
                if (d != "ellipses" && d != "polygon" || d == "polygon" && i != "slope_marquee") z += fu;
            } else z += fu;
        }
        if (v == "fill" || v == "brush" || v == "pencil" || v == "calligraphy" || v == "stamp") gui_palette.click("fill");
        else if (v == "spirograph") gui_palette.click("stroke");
        switch(v){
            case "picker":
                z = '<img src="media/gui/loupe.png" onmousedown="return(noMove())" class="loupe" alt="..."><canvas id="picker" height="106" width="149" style="height: 106px; width: 149px"></canvas><div class="picker"><div>R<br>G<br>B<br>A</div><div id="picker_hex">0<br>0<br>0<br>0</div></div><canvas id="picker1x1" height="1" width="1"></canvas>';
                break;
            case "stamp":
                z = '<div id="stamp"></div>' + gui.Y.kontrol("stamp") + '<span style="position: relative; top: -6px; border: 0" id="brush_author"></span>' + z;
                break;
            case "text":
                if (!vars.textMessage) vars.textMessage = "hello!";
                z = '<div id="text" style="padding-bottom: 13px"> <span style="font-size: 11px; -moz-user-select: none; -khtml-user-select: none; user-select: none; ">MESSAGE</span><br> <input type="text" onkeydown="vars.textMessage=this.value;" onkeyup="vars.textMessage=this.value;" style="width: 90%; font-size: 16px" value="' + vars.textMessage + '">' + "</div>" + z;
                break;
        }
        var d = $("options");
        ...
        if (v == "stamp") stamp.reset();
        else if (v == "calligraphy") co.glyph("media/glyph/Calligraphy/0-live.png");
        if (r.glyph) co.glyph();
        if (!$("ioptions").opened) win.tab($("ioptions"), "options");
    ...
    resize: function(n) {
        ...
        var o1 = $("options"), l = $C("ML", o1)[0], m = $C("MM", o1)[0], r = $C("MR", o1)[0];
        fu(l);
        fu(r);
        fu(m);
...
gui_palette = {
    ...
    zindex: function(v2) {
        ...
        fu(v2, "default", 2);
        fu(r[v2], "pointer", 1);
        $("swap").innerHTML = v2.substr(0, 1).toUpperCase();
    },
    current: function() {
        ...
        var z = vars.id == "fill", a = z ? "stroke" : "fill", b = z ? "fill" : "stroke", z = "";
...
gui_color = {
    ...
    "mk": function() {
        ...
        for(var i in R){
            ...
            z += '<div title="' + i.substr(0, 1).toUpperCase() + i.substr(1) + '">' + ' <div class="west" id="' + i + 'T">' + i.substr(0, 1).toUpperCase() + "</div>" + ' <span class="east" id="' + i + 'Me"></span>' + ' <div onmousedown="' + v + '; return false;" id="' + i + 'Cur" class="cur"></div>' + ' <canvas id="' + i + '" onmousedown="' + v + '" height="18" width="120"></canvas><br>' + "</div>";
        }
    ...
    "run": function(m, r) {
        if (m == "set") m = "Red";
        if (r) var r = gui_color.RGB = {
            R: r[0],
            G: r[1],
            B: r[2],
            A: r[3]
        }, h = gui_color.HSV = Color.RGB_HSV(gui_color.RGB);
        else var r = gui_color.RGB, h = gui_color.HSV;
        if (gui_color.HSV_[m]) {
            var t = Color.HSV_RGB(h);
            t.A = r.A;
            r = gui_color.RGB = t;
        } else if (gui_color.RGB_[m]) h = gui_color.HSV = Color.RGB_HSV(r);
        var R = {
            "Hue": [
                [
                    0,
        ...
        for(var i in R){
            ...
            c.rect(0, 0, 120, 18);
            co.style.fill(c, g);
            if (gui_color.HSV_[i]) gui_color.pos(h, "HSV", i);
            else gui_color.pos(r, "RGB", i);
        }
...
gui_gradient = {
    ...
    "slide_y": function(a, b, m) {
        var g = vars[vars.id + "GD"], op = Math.round((1 - (b.Y + 5) / 91) * 1000) / 1000;
    ...
    "slide_o": function(o, m, a, x) {
        a.X += x.oX;
        a.Y += x.oY;
        var o = $S("gAngle"), W = 132, W2 = W / 2, R = Math.atan2(a.X - W2 - 3, W - a.Y - W2 - 6);
    ...
    "run": function(v, m) {
        ...
        if (isNaN(v)) for(var i1 in g)z(i1);
        else z(v);
        gui_gradient.fill(m);
    },
    "fill": function(m) {
...
var gui_pattern = {
    ...
    "mk": function(s) {
        ...
        gui_palette.update();
        if (vars[vars.id] == "pattern") gui_gradient.cs();
    ...
    "create": function(opacity, m) {
        ...
        if (!opacity) opacity = gui_pattern.op[vars.id];
...
gui_swatch = {
    ...
    "click": function(o) {
        ...
        if (gui_swatch.n[vars.id + gui_swatch.id] != n) {
            ...
            } else vars[vars.id + gui_swatch.id] = deObject(vars[gui_swatch.id][n - 1]);
    ...
    "pos": function(n) {
        var o = gui.Y, a = Math.ceil(n / 7) * 7, b = a - 7 - (o.cur[gui_swatch.id] - 1);
    ...
    "add": function() {
        var r1 = vars[gui_swatch.id], n = gui_swatch.n[vars.id + gui_swatch.id];
        var fu = {
            "CO": function(r) {
        ...
        r1.splice(n, 0, fu[gui_swatch.id](r1));
        vars[vars.id + gui_swatch.id] = r1[n];
    ...
    author: function(id) {
        ...
        if (res) return '<i>by:&nbsp; <a href="' + res.url + '" target="_blank">' + res.name + "</a></i>";
        else return "";
    },
    "mk": function() {
        var z = "";
        ...
        for(var i in gui_swatch.r){
            ...
            z += '<div class="menu ' + i + 'menu" onmousedown="gui_swatch.cur(\'' + i + "'); vars.cache(1);\">" + " <div>" + gui_swatch.r[i][0].toUpperCase() + "S</div>" + ' <span class="east">' + ' <div style="padding: 2px 0 0; font-size: 12px" id="' + i + '_author"></div>' + //				 '  <img src="media/gui/sw_remove.png"
            ...
            " </span>" + "</div>" + '<div class="' + i + '" style="display: none">' + ' <div style="position: absolute; left: 19px; top: ' + (42 + gui_swatch.r[i][2]) + 'px;">' + gui.menu.build(i + "*", gui_swatch.set[i]) + "</div>" + ' <div id="' + i + '" class="squares"></div>' + gui.Y.kontrol(i, gui_swatch.r[i][2] + 49) + "<br>" + res + "</div>";
        }
    ...
    "update": function(i2, s) {
        var r2 = vars[gui_swatch.id];
        var fu = {
            "CO": function(r, c) {
                c.fillStyle = "rgba(" + r[i2 - 1].join(",") + ")";
                c.fill();
            },
            "GD": function(r, c) {
                co.gradient({
            ...
            "PT": function(r, c) {
                c.fillStyle = c.createPattern(r[i2 - 1], "repeat");
                c.fill();
            }
        };
        function z(i) {
        ...
        if (isNaN(i2)) for(var i2 = gui.Y.cur[gui_swatch.id], ii = 1; i2 <= r2.length && ii <= 28; i2++, ii++)z(i2);
        else z(i2);
    },
...
infc = {
    ...
    "opacity": function(o, v, n) {
        ...
        if ($(o + "CurV")) $(o + "CurV").innerHTML = Math.round(op);
        switch(vars["fill"]){
            case "pattern":
...
//# sourceMappingURL=index.6c182b8e.js.map
