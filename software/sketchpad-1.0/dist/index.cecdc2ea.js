...
(function() {
    ...
    (vars.cache = function(s) {
        const c = cookieGrab("settings");
        if (!vars.tool) vars.updateTool();
        if (c && !s) {
            var z = c.split(","); // OPEN
            for(var i in z){
                const v = z[i].split(":");
                if (`${Number.parseInt(v[1])}` === v[1]) v[1] = Number.parseInt(v[1]);
                else if (v[0] !== "zoom") vars[v[0]] = v[1];
            }
        } else {
            var z = "", r = {
                "object": 1,
            ...
            for(var i in vars)if (!r[typeof vars[i]]) z += i + ":" + vars[i] + ",";
            z = z.substr(0, z.length - 1);
            cookieStab("settings", z);
        }
    })();
    /* SHAPES */ co.polygon = function(a, b, c, s) {
        const T = !isNaN(s) ? "marquee" : "shape", o = co.constrain(a, b, s), scale = co.scale(a, b, s);
        const rad = Math.sqrt(Math.pow(a.X - o.X, 2) + Math.pow(a.Y - o.Y, 2)) / 2, sides = vars["sides_" + T];
        const x = o.X - (o.X - a.X) / 2, y = o.Y - (o.Y - a.Y) / 2, step = Math.PI * 2 / sides;
        const angle = E.ctrl ? Math.atan2(o.X - a.X, o.Y - a.Y) : Math.PI / 180 * 45, j = 0;
        const check = isNaN(s) && mXY == "up" && Math.abs(a.X - b.X) > 0 && Math.abs(a.Y - b.Y) > 0;
        function fu_x() {
        ...
        var n = 0;
        c.moveTo(fu_x(), fu_y());
        for(var n = 1; n <= sides; n++)c.lineTo(fu_x(), fu_y());
        c.closePath();
    };
    co.star = function(a, b, c, s) {
        const T = !isNaN(s) ? "marquee" : "shape", o = co.constrain(a, b, s), scale = co.scale(a, b, s);
        const rad = Math.sqrt(Math.pow(a.X - o.X, 2) + Math.pow(a.Y - o.Y, 2)) / 2, rad2 = rad / vars["slope_" + T], sides = vars["sides_" + T];
        const x = o.X - (o.X - a.X) / 2, y = o.Y - (o.Y - a.Y) / 2, step = Math.PI * 2 / sides, halfStep = step / 2;
        const r = Math.PI / 180, angle = E.ctrl ? Math.atan2(o.X - a.X, o.Y - a.Y) : r * 45, j = 0;
        const check = isNaN(s) && mXY == "up" && Math.abs(a.X - b.X) > 0 && Math.abs(a.Y - b.Y) > 0;
        function fu1(v) {
    ...
    co.burst = function(a, b, c, s) {
        const T = !isNaN(s) ? "marquee" : "shape", o = co.constrain(a, b, s), scale = co.scale(a, b, s);
        const rad = Math.sqrt(Math.pow(a.X - o.X, 2) + Math.pow(a.Y - o.Y, 2)) / 2, rad2 = rad / vars["slope_" + T], sides = vars["sides_" + T];
        const x = o.X - (o.X - a.X) / 2, y = o.Y - (o.Y - a.Y) / 2, step = Math.PI * 2 / sides, halfStep = step / 2, qtrStep = step / 4;
        const angle = E.ctrl ? Math.atan2(o.X - a.X, o.Y - a.Y) : Math.PI / 180 * 45, j = 0;
        const check = isNaN(s) && mXY == "up" && Math.abs(a.X - b.X) > 0 && Math.abs(a.Y - b.Y) > 0;
    ...
    co.ellipses = function(a, b, c, s) {
        const T = !isNaN(s) ? "marquee" : "shape", o = co.constrain(a, b, s), scale = co.scale(a, b, s);
        const rad = Math.sqrt(Math.pow(a.X - o.X, 2) + Math.pow(a.Y - o.Y, 2)) / 2, sides = 150;
        const x = o.X - (o.X - a.X) / 2, y = o.Y - (o.Y - a.Y) / 2, step = Math.PI * 2 / sides;
        const angle = E.ctrl ? Math.atan2(o.X - a.X, o.Y - a.Y) : Math.PI / 180 * 45, j = 0;
        const check = isNaN(s) && mXY == "up" && Math.abs(a.X - b.X) > 0 && Math.abs(a.Y - b.Y) > 0;
        function fu_x() {
    ...
    co.circle = function(a, b, c) {
        ...
        const r = Math.sqrt(Math.pow(a.X - b.X, 2) + Math.pow(a.Y - b.Y, 2)) / 2;
        c.arc(d.X, d.Y, r, 0, Math.PI * 2, true);
        c.closePath();
    };
    co.rectangle = function(a, b, c, s) {
        const o = co.constrain(a, b, s);
    ...
    /* CACHE */ co.brush = function() {
        const type = vars.type;
        if (type === "brush" || type === "eraser" || type === "pencil") {
            const c = $2D("ctx_brush"), D = vars[`diameter_${vars.type}`] || 10, D2 = D * 2;
            $("ctx_brush").width = D2;
            $("ctx_brush").height = D2;
            co.del(c);
            c.globalCompositeOperation = "source-over";
            const hardness = vars[`hardness_${vars.type}`] || 100;
            const opacity = vars["opacity_" + vars.type] || 1;
            const r = c.createRadialGradient(D, D, type === "pencil" ? D - 1 : hardness / 100 * D - 1, D, D, D);
            r.addColorStop(0, `rgba(0,0,0,${opacity / 100})`);
    ...
    co.glyph = function(v, o1) {
        ...
        co.stamp.image.onload = function() {
            const o = co.stamp.image;
            stamp.loaded = true;
            const id = stamp.fileNumber;
            C$("stamp" + id, {
                "-": "load"
            });
    ...
    co.stamp = function() {
        if ((vars.type == "calligraphy" || vars.type == "stamp") && co.stamp.image.src) {
            const c = $2D("ctx_stamp"), i = co.stamp.image, n = vars.type == "stamp" ? vars.rand_min / 100 : 1, o = co.stamp.r = {
    ...
    co.stamp.r = {};
    /* EXTENDED */ co.core = function(e, fu) {
        const d = win_size.LT();
        core.fu("cBound", e, {
            fu: core.XY,
            X1: 0,
    ...
    co.path = function(c, r) {
        ...
        for(let i in r)c.lineTo(r[i].X, r[i].Y);
    };
    /* TRANSFORMATION */ co.constrain = function(a, b, s) {
        if ((E.sh || !isNaN(s)) && s == 1 || E.sh && isNaN(s) && !E.ctrl) {
            const X = b.X - a.X ? b.X - a.X : 1, Y = b.Y - a.Y ? b.Y - a.Y : 1, Z = {
                X: Math.abs(X),
                Y: Math.abs(Y)
            };
            if (Z.X < Z.Y) return {
    ...
    co.scale = function(a, b, s) {
        if ((E.sh || !isNaN(s)) && s == 1 || E.sh && isNaN(s)) return {
            X: 1,
            Y: 1
        };
        else if (Math.abs(a.X - b.X) > Math.abs(a.Y - b.Y)) return {
            X: 1,
    ...
    co.gradient = function(a, b, c, r, v, s) {
        let g;
        if (!s) {
            const W = Math.sqrt(Math.pow(b.X - a.X, 2)), W2 = W / 2, rad = gui_gradient.rotate, mX = Math.min(b.X, a.X), mY = Math.min(b.Y, a.Y);
            const s1 = Math.abs(Math.sin(rad) * W2 + W2), s2 = Math.abs(Math.cos(rad) * W2 - W2);
            const x1 = Math.abs(Math.sin(rad + Math.PI) * W2 + W2), y1 = Math.abs(Math.cos(rad + Math.PI) * W2 - W2);
            g = c.createLinearGradient(s1 + mX, s2 + mY, x1 + mX, y1 + mY);
        } else g = c.createLinearGradient(a.X, a.Y, b.X, b.Y);
        for(let i in r)if (!r[i][2]) g.addColorStop(r[i][0], "rgba(" + r[i][1].join(",") + ")");
        co.style[v](c, g);
    };
})();

//# sourceMappingURL=index.cecdc2ea.js.map
