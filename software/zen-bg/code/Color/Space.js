...
var spaceX = ColorSpace = Color.SpaceX = function(color, type) {
	...
	*/
    ...
    ///
    for (var key in defs) {
        if (color.indexOf(key) === 0) {
			defs[key](this, color);
        	return this;
        }
    }
	///
	if (Color.WebColors[color]) {
		defs["#"](this, Color.WebColors[color]);
	}
	///
	return this;
};
...
spaceX.prototype.isW3Brightness = function (a, b) { // color brightness >= 125.
	var aa = ((a.r * 299) + (a.g * 587) + (a.b * 114)) / 1000;
	var bb = ((b.r * 299) + (b.g * 587) + (b.b * 114)) / 1000;
	return Math.abs(aa - bb);
};

spaceX.prototype.isW3Difference = function (a, b) { // color difference >= 500
...
root.W3Brightness = function (a, b) { // color brightness >= 125.
	var aa = ((a.R * 299) + (a.G * 587) + (a.B * 114)) / 1000;
	var bb = ((b.R * 299) + (b.G * 587) + (b.B * 114)) / 1000;
	return Math.abs(aa - bb);
};

root.W3Difference = function (a, b) { // color difference >= 500
	var r = Math.max(a.R, b.R) - Math.min(a.R, b.R);
	var g = Math.max(a.G, b.G) - Math.min(a.G, b.G);
	var b = Math.max(a.B, b.B) - Math.min(a.B, b.B);
	return r + g + b;
