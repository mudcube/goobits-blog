<script lang="ts">
  ...
  const eventsSource = $derived(
    mockMode ? mockDashboardEvents : dashboard.events,
  );
  ...
  $effect(() => {
    ...
    if (!found) {
      void goto(
        mockMode ? "/schedule/admin/events/?mock=1" : "/schedule/admin/events/",
      );
      return;
    }
    if (!mockMode) {
      ...
      dashboard.programDraft = {
        ...
        heroTitleLine1:
          found.heroTitleLines[0] || dashboard.programDraft.heroTitleLine1,
        heroTitleLine2:
          found.heroTitleLines[1] || dashboard.programDraft.heroTitleLine2,
  ...
  function updateProgramField(
    field: keyof typeof dashboard.programDraft,
    value: string,
  ) {
    dashboard.programDraft = {
      ...dashboard.programDraft,
  ...
  function computeRepeatWeeks(startDate: Date) {
    ...
    return Math.max(
      0,
      Math.min(52, Math.floor(diffMs / (7 * 24 * 60 * 60 * 1000))),
    );
  }
  ...
  onMount(() => {
    ...
    window.addEventListener(
      "admin-program-editor-toggle-settings",
      onTopbarToggleSettings,
    );
    return () => {
    ...
    <div class="program-editor__canvas-wrap">
      <div class="program-editor__canvas">
        ...
        {#if popOpen}
          ...
          >
            ...
            <div class="program-editor__popover-title">
              {#if selectedDayDate}{selectedDayDate.toLocaleDateString(
                  undefined,
            ...
            {#if selectedDayDate && !activeDays[isoDay(selectedDayDate)]}
              <div class="program-editor__opt-row">
                <button
                  ...
                  onclick={() => (newMode = "once")}>Just this day</button
                >
                <button
                  type="button"
                  class="program-editor__opt"
                  class:program-editor__opt--on={newMode === "repeat"}
                  onclick={() => (newMode = "repeat")}>Repeat weekly</button
                >
              </div>
              ...
              {#if newMode === "repeat"}
                <div class="program-editor__until">
                  <button
                    type="button"
                    class:program-editor__until-btn--on={untilMode ===
                      "ongoing"}
                    class="program-editor__until-btn"
                    onclick={() => (untilMode = "ongoing")}>Ongoing</button
                  >
                  <button
                    type="button"
                    class:program-editor__until-btn--on={untilMode === "date"}
                    class="program-editor__until-btn"
                    onclick={() => (untilMode = "date")}>Pick date</button
                  >
                </div>
                {#if untilMode === "date"}
                  <input
              ...
              <div class="program-editor__actions">
                <AdminActionButton variant="subtle" onclick={closePop}
                  >Cancel</AdminActionButton
                >
                <AdminActionButton
                  variant="primary"
                  onclick={() => void persistDaySchedule()}
            ...
            {:else}
              ...
              {#if selectedDayDate && activeDays[isoDay(selectedDayDate)]?.repeatLabel}
                <div class="program-editor__override">
                  <div class="program-editor__override-label">
                    Part of repeating schedule
                  </div>
                  <div class="program-editor__override-text">
                    Changes here apply to this day preview only. Save a new
                    schedule to persist.
                  </div>
                </div>
              {/if}
              <div
                class="program-editor__actions program-editor__actions--split"
              >
                <AdminActionButton
                  variant="danger"
                  onclick={() => void removeDay()}>Remove</AdminActionButton
                >
                <AdminActionButton
                  variant="primary"
                  onclick={() => void persistExistingDayEdits()}
                  >Done</AdminActionButton
                >
              </div>
            {/if}
          </div>
        ...
        <p class="program-editor__hint">
          <Lightbulb
            class="program-editor__hint-icon"
            size={14}
            strokeWidth={1.9}
            aria-hidden="true"
          />
          ...
          <div class="program-editor__settings-body">
            <div class="program-editor__toggle-row">
              ...
              <button
                ...
                class:program-editor__switch--on={dashboard.programDraft
                  .enabled}
                onclick={() =>
                  (dashboard.programDraft = {
                    ...dashboard.programDraft,
...
<style>
  ...
  .program-editor__popover {
    ...
    --popover-control-bg: color-mix(
      in srgb,
      #faf6ff 88%,
      var(--popover-surface) 12%
    );
