<script lang="ts">
  ...
  const footerNav = [
    { href: "/schedule/admin/settings/", label: "Settings", icon: Settings },
  ];
  ...
  function active(path: string) {
    ...
    const current =
      $page.url.pathname.endsWith("/") && $page.url.pathname.length > 1
        ? $page.url.pathname.slice(0, -1)
        : $page.url.pathname;
    if (normalizedPath === "/schedule/admin")
      return current === "/schedule/admin";
    return (
  ...
  function adminSectionTitle(pathname: string) {
    if (
      pathname === "/schedule/admin/crew" ||
      pathname.startsWith("/schedule/admin/crew/")
    )
      return "Crew";
    if (
      pathname === "/schedule/admin/events" ||
      pathname.startsWith("/schedule/admin/events/")
    )
      return "Events";
    if (
      pathname === "/schedule/admin/settings" ||
      pathname.startsWith("/schedule/admin/settings/")
    )
      return "Settings";
    if (
      pathname === "/schedule/admin/config" ||
      pathname.startsWith("/schedule/admin/config/")
    )
      return "Settings";
    return "Dashboard";
  }
  ...
  function breadcrumbs(pathname: string) {
    ...
    const eventsSingleSegment = normalized.match(
      /^\/schedule\/admin\/events\/([^/]+)$/,
    );
    const crewSingleSegment = normalized.match(
      /^\/schedule\/admin\/crew\/([^/]+)$/,
    );
    ...
    if (crewSingleSegment)
      return [
        ...items,
        { label: "Crew", href: hrefWithMock("/schedule/admin/crew/") },
        { label: prettyLeaf(crewLeaf) },
      ];
    if (normalized.startsWith("/schedule/admin/crew"))
      return [...items, { label: "Crew" }];
    if (
      normalized === "/schedule/admin/settings" ||
      normalized.startsWith("/schedule/admin/settings/")
    )
      return [...items, { label: "Settings" }];
    if (normalized === "/schedule/admin/events")
      return [...items, { label: "Events" }];
  ...
  function triggerProgramEditorSettings() {
    window.dispatchEvent(
      new CustomEvent("admin-program-editor-toggle-settings"),
    );
  }
...
<svelte:head>
  <title
    >{adminSectionTitle($page.url.pathname)} | {calendarConfig.brand
      .calendarName} | {calendarConfig.brand.siteName}</title
  >
</svelte:head>
...
<div class="social-admin">
        ...
        >
          {#if isProgramEditorRoute($page.url.pathname)}
            ...
            <AdminActionButton
              variant="subtle"
              icon={Settings}
              onclick={triggerProgramEditorSettings}>Settings</AdminActionButton
            >
          {:else if isEventsIndexRoute($page.url.pathname)}
            <AdminActionButton
              variant="primary"
              icon={CalendarPlus}
              href={hrefWithMock("/schedule/admin/events/new/")}
          ...
          {:else if isCrewRoute($page.url.pathname)}
            <AdminActionButton
              variant="primary"
              icon={UserPlus}
              onclick={triggerCrewInvite}>Invite</AdminActionButton
            >
          {:else if isDashboardRoute($page.url.pathname)}
            <AdminActionButton
              variant="subtle"
              icon={CalendarDays}
              href="/schedule/">Calendar</AdminActionButton
            >
          {:else if isEventDetailRoute($page.url.pathname)}
            <AdminActionButton
              variant="primary"
              icon={Pencil}
              onclick={triggerEventDetailEdit}>Edit</AdminActionButton
            >
            <AdminActionButton
              variant="danger"
              icon={Trash2}
              onclick={triggerEventDetailCancel}>Cancel Event</AdminActionButton
            >
          {:else}
  ...
  <aside class="social-admin__sidebar">
    <a
      class="social-admin__brand"
      href={hrefWithMock("/schedule/admin/events/")}>Admin</a
    >
    ...
    {#if data.user}
      <form
        class="social-admin__logout"
        method="POST"
        action="/schedule/admin?/logout"
        use:enhance
      >
        <button type="submit">
...
<style>
  .social-admin {
    ...
    --admin-control-border: color-mix(
      in srgb,
      var(--admin-accent) 34%,
      transparent
    );
    --admin-control-bg: color-mix(
      in srgb,
      var(--admin-accent) 10%,
      transparent
    );
    --admin-control-bg-hover: color-mix(
      in srgb,
      var(--admin-accent) 15%,
      transparent
    );
    --admin-control-fg: color-mix(in srgb, var(--text) 92%, transparent);
    --admin-control-primary-bg: color-mix(
      in srgb,
      var(--admin-accent) 10%,
      transparent
    );
    --admin-control-primary-fg: color-mix(
      in srgb,
      var(--text) 92%,
      transparent
    );
    --admin-control-danger-bg: color-mix(
      in srgb,
      var(--admin-accent) 10%,
      transparent
    );
    ...
    --admin-card-border: color-mix(
      in srgb,
      var(--admin-accent) 18%,
      transparent
    );
    --admin-card-bg-hover: color-mix(
      in srgb,
      var(--admin-accent) 10%,
      var(--bg) 90%
    );
    ...
    --admin-toast-success-bg: color-mix(
      in srgb,
      var(--admin-accent) 14%,
      var(--bg) 86%
    );
    --admin-toast-success-border: color-mix(
      in srgb,
      var(--admin-accent) 34%,
      transparent
    );
    ...
    --admin-elev-control-hover: color-mix(
      in srgb,
      var(--text) 72%,
      var(--bg) 28%
    );
    --admin-elev-control-active: color-mix(
      in srgb,
      var(--admin-accent) 44%,
      var(--text) 56%
    );
    ...
    --admin-calendar-arrow-border: color-mix(
      in srgb,
      var(--text) 22%,
      transparent
    );
    --admin-calendar-arrow-bg: color-mix(
      in srgb,
      var(--panel-bg) 84%,
      var(--text) 16%
    );
    ...
    --admin-calendar-arrow-hover-bg: color-mix(
      in srgb,
      var(--admin-accent) 14%,
      var(--panel-bg) 86%
    );
    --admin-calendar-arrow-hover-fg: color-mix(
      in srgb,
      var(--admin-accent) 80%,
      var(--text) 20%
    );
    ...
    --admin-calendar-grid-border: color-mix(
      in srgb,
      var(--text) 12%,
      transparent
    );
    --admin-calendar-cell-border: color-mix(
      in srgb,
      var(--text) 10%,
      transparent
    );
    --admin-calendar-day-hover: color-mix(in srgb, var(--text) 5%, transparent);
    --admin-calendar-today-bg: color-mix(
      in srgb,
      var(--admin-accent) 25%,
      transparent
    );
    --admin-calendar-selected-border: color-mix(
      in srgb,
      var(--admin-accent) 62%,
      transparent
    );
    --admin-calendar-selected-bg: color-mix(
      in srgb,
      var(--admin-accent) 11%,
      transparent
    );
    --admin-calendar-selected-ring: color-mix(
      in srgb,
      var(--admin-accent) 16%,
      transparent
    );
    --admin-calendar-dot: color-mix(
      in srgb,
      var(--admin-accent) 76%,
      var(--text) 24%
    );
  ...
  :global(.social-admin .ui-form-control) {
    width: 100%;
    min-height: 36px;
    padding: 0 0.72rem;
    border-radius: 8px;
  ...
  :global(.social-admin .ui-form-control:focus) {
    outline: none;
    border-color: var(--admin-selected-border);
    box-shadow: 0 0 0 2px
      color-mix(in srgb, var(--admin-focus-ring) 45%, transparent);
  ...
  .social-admin__nav-item--active {
    ...
    box-shadow: inset 0 0 0 1px
      color-mix(in srgb, var(--admin-accent) 34%, transparent);
  }
      ...
      var(--admin-content-pad-x) +
        (
          min(
              ...
              calc(100% - (var(--admin-content-pad-x) * 2))
            ) /
            2
        )
    );
