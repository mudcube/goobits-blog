<script lang="ts">
	...
	import { filterAndSortDirectoryItems, type DirectoryItem, type DirectorySort } from '$lib/viewmodels/directory'
	...
	const accentColors = ['#ec4899', '#f59e0b', '#eab308', '#22c55e', '#14b8a6', '#3b82f6', '#8b5cf6', '#f43f5e', '#f97316']
	...
	const filteredItems = $derived(filterAndSortDirectoryItems(items, searchQuery, sortBy))
</script>
...
<PageShell className="directory-page">
	<div class="directory-page__inner">
		...
		<section class="directory-page__toolbar" aria-label={`${title} filters`}>
			<label class="directory-page__search" aria-label={`Search ${itemLabel}s`}>
				<span class="directory-page__search-icon" aria-hidden="true">
					<Search size={15} strokeWidth={2.2} />
				</span>
				<input class="directory-page__search-input" type="text" placeholder={`Search ${itemLabel}s...`} bind:value={searchQuery} />
			</label>

			<div class="directory-page__chip-group" role="tablist" aria-label={`Sort ${title}`}>
			...
			<ul class="directory-page__grid" aria-label={title}>
				{#each filteredItems as item}
					<li class="directory-page__item">
						<a href={item.href} class="directory-page__card" style={`--accent:${getAccentColor(item)};`}>
							...
							<div class="directory-page__card-body">
								<div class="directory-page__card-head">
									...
									{#if item.date}
										...
										<span class="directory-page__date" title={formatDateMmDdYyyy(parsed)}>
											{parsed.getFullYear()}
										</span>
									{/if}
								</div>
...
<style>
	...
	.directory-page__search:focus-within {
		border-color: color-mix(in srgb, var(--text) 55%, var(--border));
	}

	.directory-page__search-icon {
		color: color-mix(in srgb, var(--muted) 92%, var(--text));
		flex-shrink: 0;
	}

	.directory-page__search-input {
		width: 100%;
	...
	.directory-page__search-input:focus {
		outline: none;
	}
