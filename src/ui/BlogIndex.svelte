<script lang="ts">
	...
	const filterNavigation = createBlogFilterNavigation(
		navigateAdapter ? { navigate: navigateAdapter } : {}
	)

	function getPageHref(pageNumber: number): string {
		if (pageHref) {
			return pageHref(pageNumber)
		}
	...
	async function loadNextPage(): Promise<void> {
		if (!loadPage || loading || !canLoadMore) {
			return
		}
		loading = true
		try {
...
<section
	class={['blog-index', className].filter(Boolean).join(' ')}
...
>
	<header class="blog-index__header">
		...
		{#if categories.length > 0 || tags.length > 0}
			<nav class="blog-index__discovery" aria-label={messages.blogTopics}>
				{#each categories as category (category.slug)}
					<a href={urlResolver.taxonomy('category', category.slug, config)}
						>{category.name} <span>{category.count}</span></a
					>
				{/each}
				{#each tags as tag (tag.slug)}
					<a href={urlResolver.taxonomy('tag', tag.slug, config)}
						>#{tag.name} <span>{tag.count}</span></a
					>
				{/each}
			</nav>
		{/if}
		<form
			...
			method="GET"
			aria-label={messages.searchAndSort}
	...
	{#if paginationMode === 'pages' && totalPages > 1}
		<nav class="blog-index__pagination" aria-label={messages.blogPages}>
			{#if currentPage > 1}<a href={getPageHref(currentPage - 1)} rel="prev"
					>{messages.previousPage}</a
				>{/if}
			<span>{messages.pageStatus(currentPage, totalPages)}</span>
			{#if currentPage < totalPages}<a href={getPageHref(currentPage + 1)} rel="next"
					>{messages.nextPage}</a
				>{/if}
		</nav>
	{:else if canLoadMore}
