...
<article class={['blog-post', className].filter(Boolean).join(' ')}>
	...
	<header class="blog-post__header">
		{#if post.categories.length > 0}
			<TagCategoryList items={post.categories} type="categories" {config} messages={messageInput} urlResolver={urlResolverInput} />
		{/if}
		...
		<div class="blog-post__meta">
			...
			{#if post.author}<Byline author={post.author} href={urlResolver.author(post.author, config)} layout="inline" />{/if}
		</div>
	...
	<footer class="blog-post__footer">
		{#if post.tags.length > 0}<TagCategoryList items={post.tags} {config} messages={messageInput} urlResolver={urlResolverInput} />{/if}
		<SocialShare url={canonicalUrl} title={post.title} text={post.excerpt} networks={shareNetworks} messages={messageInput} />
		<a class="blog-post__back" href={urlResolver.blog(config)}><ArrowLeft size={16} aria-hidden="true" /> {messages.backToBlog}</a>
	</footer>
		...
		<section class="blog-related" aria-labelledby="blog-related-title">
			...
			<div class="blog-related__grid">
				{#each related as relatedPost (relatedPost.id)}
					<BlogCard post={relatedPost} {config} compact messages={messageInput} urlResolver={urlResolverInput} {locale} />
				{/each}
			</div>
		</section>
	{/if}
