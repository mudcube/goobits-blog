...
export function createOtpAuthURL({ secret = "", label = "", issuer = "", digits = 6, period = 30, } = {}) {
...
export async function generateTOTP({ secret = "", time = Date.now(), digits = 6, period = 30, } = {}) {
    if (!secret) {
...
export async function verifyTOTP({ secret = "", token = "", digits = 6, period = 30, window = 1, time = Date.now(), } = {}) {
    if (!secret || !token)
        return false;
