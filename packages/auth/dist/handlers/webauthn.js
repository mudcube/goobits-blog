...
const registrationResponseSchema = z.custom((value) => typeof value === "object" &&
    value !== null &&
    typeof value["id"] === "string" &&
    ((typeof value["rawId"] === "string" &&
        value["type"] === "public-key") ||
        !(("rawId" in value) || ("type" in value))));
const authenticationResponseSchema = z.custom((value) => typeof value === "object" &&
    value !== null &&
    typeof value["id"] === "string" &&
    ((typeof value["rawId"] === "string" &&
        value["type"] === "public-key") ||
...
function credentialDescriptorFromRecord(cred) {
    ...
    if (transports !== undefined && transports !== null) {
        ...
        const filtered = transports.filter((entry) => entry === "ble" ||
            entry === "cable" ||
            entry === "hybrid" ||
            entry === "internal" ||
            entry === "nfc" ||
...
export function createWebAuthnRegisterOptionsHandler(config) {
    const { webauthnAdapter, rpName, rpID, timeout = 60_000, attestationType = "none", authenticatorSelection, supportedAlgorithmIDs, userVerification: _userVerification = "preferred", getUser = (event) => event.locals.user ?? null, } = config;
    if (!rpID || !rpName) {
...
export function createWebAuthnRegisterVerifyHandler(config) {
    ...
    return async (event) => {
        ...
        const regCredentialRecord = (typeof registrationInfoRecord["credential"] === "object" &&
        ...
        await webauthnAdapter.createCredential({
            ...
            publicKey,
            counter,
            transports: credential.response && "transports" in credential.response
...
export function createWebAuthnLoginVerifyHandler(config) {
    const { webauthnAdapter, databaseAdapter, sessionAdapter, rpID, origin, redirectAfterLogin = "/", requireUserVerification = false, onLogin, sanitizeUser = defaultSanitizeUser, autoCreateSession = true, onLoginMode = "augment", } = config;
    if (!rpID || !origin) {
