...
async function emitDenied(context, message, details = {}) {
    if (!context.emitter)
        return;
    await context.emitter(createAuthEvent({
        name: "authz.denied",
        severity: "warn",
...
export function requireAuthenticated(locals) {
    if (!locals.user) {
        throw new Error("Unauthorized");
    }
}
export async function requireRole(context, requiredRoles) {
...
export async function requireOwnership(context, resourceOwnerId) {
    requireAuthenticated(context.event.locals);
