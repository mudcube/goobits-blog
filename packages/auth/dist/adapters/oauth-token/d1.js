...
function parseOAuthTokens(raw) {
    try {
        ...
        if (record["scope"] !== null && typeof record["scope"] !== "string")
            return null;
...
export class D1TokenAdapter extends TokenAdapter {
    ...
    async storeTokens(userId, provider, tokens) {
        ...
        await this.db
            .prepare(`DELETE FROM ${this.tokensTable} WHERE ${this.columns.userId} = ? AND ${this.columns.provider} = ?`)
            .bind(userId, provider)
            .run();
        await this.db
            .prepare(`INSERT INTO ${this.tokensTable} (${this.columns.userId}, ${this.columns.provider}, ${this.columns.tokens}) VALUES (?, ?, ?)`)
            .bind(userId, provider, tokenData)
            .run();
    }
    async getTokens(userId, provider) {
        const row = await this.db
            .prepare(`SELECT ${this.columns.tokens} as tokens FROM ${this.tokensTable} WHERE ${this.columns.userId} = ? AND ${this.columns.provider} = ? LIMIT 1`)
            .bind(userId, provider)
            .first();
        if (!row)
            return null;
        ...
        if (typeof tokenValue !== "string")
            return null;
        return this.encrypt
            ? await decryptTokens(tokenValue, key)
