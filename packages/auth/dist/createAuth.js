...
import { createMagicLinkRequestHandler, createMagicLinkVerifyHandler, } from "./handlers/magic-link.js";
import { createWebAuthnRegisterOptionsHandler, createWebAuthnRegisterVerifyHandler, createWebAuthnLoginOptionsHandler, createWebAuthnLoginVerifyHandler, } from "./handlers/webauthn.js";
import { createSessionListHandler, createSessionRevokeHandler, } from "./handlers/sessions.js";
...
function resolveSecurity(config) {
    ...
    const webhookUrl = typeof process !== "undefined" ? process.env["SECURITY_WEBHOOK_URL"] : undefined;
    const webhookSecret = typeof process !== "undefined" ? process.env["SECURITY_WEBHOOK_SECRET"] : undefined;
    const alerter = merged.alerts?.enabled === false
        ? null
...
function createHandlers(config, defaults, security) {
    const { adapters, providers = {}, hooks = {}, magicLink, webauthn, sessions, sanitizeUser = (user) => user, } = config;
    ...
    if (hasProviders) {
        ...
        const callbackConfig = {
            providers: Object.fromEntries(Object.entries(providers).map(([name, providerConfig]) => [name, providerConfig.provider])),
            redirectAfterLogin: urlConfig.afterLogin,
            isAuthenticated,
...
function buildRoutes(handlers) {
    return {
        login: () => {
            if (!handlers.login)
                throw new Error("OAuth login handler not configured");
