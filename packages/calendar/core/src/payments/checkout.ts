...
import { deleteConnection, getConnection, saveConnection } from '../storage/d1.ts'
...
function normalizeEnvironment(value: string | null | undefined): 'sandbox' | 'live' {
...
function parseSquareScope(scope: string | null | undefined) {
	...
	try {
		const parsed = JSON.parse(scope) as { locationId?: unknown; environment?: unknown }
		return {
			locationId: typeof parsed.locationId === 'string' ? parsed.locationId : '',
			environment: normalizeEnvironment(typeof parsed.environment === 'string' ? parsed.environment : 'sandbox')
		}
	} catch {
...
export async function getStoredPayPalPaymentCredentials({
	...
	const connection = await getConnection({ db, provider: 'paypal_checkout', base64Key })
	...
	const paypalClientId =
		...
		''
	const paypalClientSecret = getEnv(env, 'PAYPAL_CLIENT_SECRET', '')?.trim() || ''
	if (!paypalClientId || !paypalClientSecret) return null
	return {
		clientId: paypalClientId,
		clientSecret: paypalClientSecret,
		environment: normalizeEnvironment(getEnv(env, 'PAYPAL_ENVIRONMENT', 'sandbox')),
		source: 'env'
	}
}
...
export async function getStoredSquarePaymentCredentials({
	...
	const connection = await getConnection({ db, provider: 'square_cashapp', base64Key })
	if (connection?.accessToken && connection.refreshToken) {
	...
	const squareApplicationId = getEnv(env, 'PUBLIC_SQUARE_APPLICATION_ID', '')?.trim() || ''
	const squareLocationId =
		getEnv(env, 'PUBLIC_SQUARE_LOCATION_ID', '')?.trim() ||
		getEnv(env, 'SQUARE_LOCATION_ID', '')?.trim() ||
		''
	const squareAccessToken = getEnv(env, 'SQUARE_ACCESS_TOKEN', '')?.trim() || ''
	if (!squareApplicationId || !squareLocationId || !squareAccessToken) return null
	return {
		applicationId: squareApplicationId,
		locationId: squareLocationId,
		accessToken: squareAccessToken,
		environment: normalizeEnvironment(getEnv(env, 'SQUARE_ENVIRONMENT', 'sandbox')),
		source: 'env'
	}
}
...
export async function deletePaymentCredentials(db: D1DatabaseLike, provider: 'paypal' | 'square') {
	await deleteConnection({ db, provider: provider === 'paypal' ? 'paypal_checkout' : 'square_cashapp' })
}
...
export async function getPaymentCheckoutContext(
	...
	const row = await db
		...
		.bind(input.eventId, input.userId, input.confirmationId ?? null, input.confirmationId ?? null)
		.first<{
			event_id: number
			title: string
...
async function getPayPalAccessToken(credentials: PayPalPaymentCredentials) {
	...
	const res = await fetch(`${paypalApiBase(credentials.environment)}/v1/oauth2/token`, {
		method: 'POST',
		headers: {
...
export async function createPayPalCheckoutOrder({
	...
	const credentials = await getStoredPayPalPaymentCredentials({ db, env, base64Key })
	if (!credentials) throw new Error('PayPal checkout credentials are not configured')
			...
			purchase_units: [
				{
					...
					custom_id: context.confirmationId || `${context.eventId}:${context.userId}`,
...
export async function capturePayPalCheckoutOrder({
	...
	if (!credentials) throw new Error('PayPal checkout credentials are not configured')
	const accessToken = await getPayPalAccessToken(credentials)
	const res = await fetch(`${paypalApiBase(credentials.environment)}/v2/checkout/orders/${encodeURIComponent(orderId)}/capture`, {
		method: 'POST',
		headers: {
			authorization: `Bearer ${accessToken}`,
			'content-type': 'application/json',
...
export async function createSquareCashAppPayment({
	...
	const base64Key = requireEnv(env, 'TOKEN_ENC_KEY')
	const credentials = await getStoredSquarePaymentCredentials({ db, env, base64Key })
	if (!credentials) throw new Error('Cash App Pay credentials are not configured')
	const idempotencyKey = `cashapp-${context.eventId}-${context.userId}-${context.confirmationId || Date.now()}`
	const res = await fetch(`${squareApiBase(credentials.environment)}/v2/payments`, {
		method: 'POST',
		headers: {
		...
		body: JSON.stringify({
			...
			reference_id: context.confirmationId || `${context.eventId}:${context.userId}`
		})
	})
	const body = await res.json().catch(() => null) as SquarePaymentResponse | null
