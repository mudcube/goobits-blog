...
import { addWeeksInTimezone, VENUE_TIMEZONE } from '../../src/config/venue'
...
describe('addWeeksInTimezone (venue tz)', () => {
	it('returns input unchanged for weeks=0', () => {
		const iso = '2026-05-09T16:00:00.000Z'
		expect(addWeeksInTimezone(iso, 0, VENUE_TIMEZONE)).toBe(iso)
	})
	...
	it('preserves wall clock across normal weeks (no DST transition)', () => {
		...
		const result = addWeeksInTimezone(start, 2, VENUE_TIMEZONE)
	...
	it('preserves wall clock across spring-forward DST transition', () => {
		...
		const result = addWeeksInTimezone(start, 1, VENUE_TIMEZONE)
	...
	it('handles multi-week recurrence that crosses DST', () => {
		...
		const week5 = addWeeksInTimezone(start, 5, VENUE_TIMEZONE)
		expect(localHourMin(week5)).toBe('09:00')
	})
})

describe('addWeeksInTimezone (other timezones)', () => {
