...
export async function runAdminPeopleAccessSmoke() {
	await withAdminPage(async (_page, context) => {
		const usersRes = await context.request.get(`${BASE_URL}/api/admin/users`)
		...
		const accessRes = await context.request.get(`${BASE_URL}/api/admin/users/${userId}/access`)
		...
		const putRes = await context.request.put(`${BASE_URL}/api/admin/users/${userId}/access`, {
			headers: { origin: BASE_URL, 'content-type': 'application/json' },
			data: { access: changed }
		})
		if (!putRes.ok()) throw new Error(`access PUT failed: ${putRes.status()}`)

		await context.request.put(`${BASE_URL}/api/admin/users/${userId}/access`, {
			headers: { origin: BASE_URL, 'content-type': 'application/json' },
