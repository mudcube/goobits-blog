...
const AdminPaymentIntegrationsSchema = z.object({
	paypal: z.object({
		...
		environment: z.union([z.literal('sandbox'), z.literal('live')]).optional().default('sandbox'),
		source: z.union([z.literal('stored'), z.literal('env'), z.null()]).optional().default(null),
		enabled: z.boolean()
	}),
	square: z.object({
...
const AdminStatusResponseSchema = z.object({
	...
	sync: z.object({
		activeProvider: z.union([z.literal('google'), z.literal('outlook'), z.literal('apple'), z.null()]),
		providers: z.object({
			google: z.object({
				connected: z.boolean(),
				expired: z.boolean(),
	...
	oauth: z.object({
		...
		outlookRedirectUri: z.union([z.string(), z.null()]).optional().default(null),
...
export type AdminPaymentIntegrationsResponse = z.infer<typeof AdminPaymentIntegrationsResponseSchema>
...
export async function disconnectAdminOutlookIntegration() {
	return requestApi<AdminMutationOk>(withAdminApi('/integrations/outlook/disconnect'), {
		method: 'POST',
		parse: (payload) => AdminMutationOkSchema.parse(payload)
	})
}
...
export async function disconnectAdminAppleIntegration() {
	return requestApi<AdminMutationOk>(withAdminApi('/integrations/apple/disconnect'), {
		method: 'POST',
		parse: (payload) => AdminMutationOkSchema.parse(payload)
	})
}

export async function connectAdminAppleIntegration(input: { username: string; appPassword: string; calendarUrl: string }) {
...
export async function getAdminPaymentIntegrations() {
	return requestApi<AdminPaymentIntegrationsResponse>(withAdminApi('/integrations/payments'), {
...
export async function deleteAdminPaymentIntegration(provider: 'paypal' | 'square') {
	return requestApi<AdminMutationOk>(withAdminApi(`/integrations/payments?provider=${encodeURIComponent(provider)}`), {
		method: 'DELETE',
		parse: (payload) => AdminMutationOkSchema.parse(payload)
	})
}
