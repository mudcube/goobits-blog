...
<style>
	...
	.st-prompt { position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); width: min(20rem, calc(100vw - 2.5rem)); padding: 1.25rem 1.25rem 1rem; border-radius: 0.75rem; background: rgba(16, 16, 28, 0.95); backdrop-filter: blur(16px); border: 1px solid color-mix(in srgb, var(--book-accent, #a78bfa) 20%, transparent); box-shadow: 0 16px 48px rgba(0, 0, 0, 0.5); animation: st-prompt-in 0.3s cubic-bezier(0.16, 1, 0.3, 1); pointer-events: auto; }
	...
	.st-prompt__title { margin: 0 0 0.35rem; font-family: var(--font-display); font-size: 1.15rem; font-weight: 500; color: var(--color-white, #fff); letter-spacing: -0.02em; }
	...
	.st-prompt__go { padding: 0.4rem 0.85rem; border: none; border-radius: 0.5rem; background: var(--book-confirm, #22c55e); color: var(--color-white, #fff); font: inherit; font-size: 0.72rem; font-weight: 700; cursor: pointer; transition: background 150ms; }
	.st-prompt__go:hover { background: var(--book-confirm-hover, #16a34a); }

	.st-ring { position: absolute; border: 1.5px solid color-mix(in srgb, var(--book-accent, #a78bfa) 60%, transparent); pointer-events: none; animation: st-pulse 2s ease-in-out infinite; opacity: 1; transition: opacity 0.2s ease; box-shadow: 0 0 12px color-mix(in srgb, var(--book-accent, #a78bfa) 15%, transparent); }
	.st-ring--hidden { opacity: 0; }
	@keyframes st-pulse { 0% { box-shadow: 0 0 0 0 color-mix(in srgb, var(--book-accent, #a78bfa) 35%, transparent); } 25% { box-shadow: 0 0 0 4px color-mix(in srgb, var(--book-accent, #a78bfa) 10%, transparent); } 50% { box-shadow: 0 0 0 8px color-mix(in srgb, var(--book-accent, #a78bfa) 0%, transparent); } 100% { box-shadow: 0 0 0 0 color-mix(in srgb, var(--book-accent, #a78bfa) 0%, transparent); } }

	.st-tip { position: fixed; width: max-content; max-width: min(20rem, calc(100vw - 2rem)); padding: 0.75rem 1rem; border-radius: 0.5rem; background: rgba(16, 16, 28, 0.95); backdrop-filter: blur(12px); border: 1px solid color-mix(in srgb, var(--book-accent, #a78bfa) 20%, transparent); box-shadow: 0 8px 32px rgba(0, 0, 0, 0.4); pointer-events: auto; opacity: 1; transition: opacity 0.2s ease; }
	.st-tip--hidden { opacity: 0; pointer-events: none; }
	.st-tip::before { content: ''; position: absolute; left: 50%; transform: translateX(-50%); width: 0; height: 0; border-left: 8px solid transparent; border-right: 8px solid transparent; filter: drop-shadow(0 0 3px color-mix(in srgb, var(--book-accent, #a78bfa) 15%, transparent)); }
	...
	.st-tip__btn { padding: 0.3rem 0.75rem; border: none; border-radius: 0.5rem; background: var(--book-accent); color: var(--color-white, #fff); font: inherit; font-size: 0.68rem; font-weight: 700; letter-spacing: 0.03em; cursor: pointer; transition: all 150ms; }
