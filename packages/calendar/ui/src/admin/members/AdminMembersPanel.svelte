...
<div class="admin-page__section">
    ...
    <div class="admin-page__fields-row admin-page__fields-row--invite">
      <div class="admin-page__field admin-page__field--email">
        ...
        <input
          ...
          type="email"
          bind:value={members.inviteEmail}
      ...
      <div class="admin-page__field admin-page__field--uses">
        ...
        <div class="ui-form-affix">
          <input
            ...
            type="number"
            min="1"
            bind:value={members.inviteUses}
          />
        </div>
      </div>
      <div class="admin-page__field admin-page__field--expires">
        <label class="admin-page__field-label" for="invite-expires"
          >Expires in</label
        >
        <div class="ui-form-affix">
          <input
            id="invite-expires"
            class="ui-form-control ui-form-control--number ui-form-control--days"
            type="number"
            min="1"
            bind:value={members.inviteExpires}
          />
          <span class="ui-form-affix__suffix">days</span>
        </div>
      </div>
    </div>
  </div>
  <PillButton
    ...
    variant="secondary"
    onClick={members.createInvite}
    disabled={members.creating}
  >
    {#if members.creating}
        ...
        <div class="admin-page__members-row">
          <div class="admin-page__members-main">
            ...
            <div class="admin-page__members-meta">
              {invite.uses_remaining ?? "∞"} use{invite.uses_remaining === 1
                ? ""
                : "s"} left
              {#if invite.email}
                · {invite.email}{/if}
              {#if invite.expires_at}
                · expires {formatDate(invite.expires_at)}{/if}
            </div>
          </div>
          <div class="admin-page__members-actions">
            <PillButton
              ...
              variant="secondary"
              size="sm"
              onClick={() => members.copyInvite(invite.code)}
            >
              Copy Link
            </PillButton>
            <PillButton
              ...
              variant="danger"
              size="sm"
              onClick={() => members.deleteInvite(invite.id)}
            >
              Delete
            </PillButton>
          </div>
        </div>
        {#if i < members.invites.length - 1}<div
  ...
  {:else}
    <div class="admin-page__members-list">
      {#each members.users.filter((user) => {
            ...
            <div class="admin-page__members-user">
              {#if user.avatar_url}
                <img
                  src={user.avatar_url}
                  alt=""
                  class="admin-page__members-avatar"
                />
              {:else}
                <span
                  class="admin-page__members-avatar-fallback"
                  aria-hidden="true">{initialsFor(user)}</span
                >
              {/if}
              <span class="admin-page__members-user-name"
                >{user.name || user.email}</span
              >
            </div>
            <div class="admin-page__members-meta">
              {user.provider || "member"} · last login {formatDate(
                user.last_login_at,
              )}
            </div>
          </div>
        ...
        {#if i < members.users.length - 1}<div
            class="admin-page__booking-divider"
          ></div>{/if}
      {/each}
    </div>
    ...
    {:else}
      <div class="admin-page__members-list">
        {#each members.accessRows as row, i}
          ...
          {#if i < members.accessRows.length - 1}<div
              class="admin-page__booking-divider"
            ></div>{/if}
        {/each}
      </div>
      <div class="admin-page__button-row">
        ...
        <PillButton
          className="admin-page__button-secondary"
          variant="secondary"
          onClick={members.saveAccess}
          disabled={members.accessSaving}
        >
