...
Site-wide UI primitives shared across miko-art surfaces. This package owns the
"site" design system — buttons, forms, layout shells, navigation, and the
showcase grid used on the landing/art/music pages.
...
3. Re-export from `src/index.ts` so it's reachable via `@miko/ui`.
...
- Components consume site design tokens (`--color-*`, `--spacing-*`) — never
  hard-code colors or sizes when a token exists.
...
`svelte.config.js` aliases `@miko/ui` to `./packages/ui/src`, and the package
